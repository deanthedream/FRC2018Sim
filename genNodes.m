...
pcw = pch;%power cube width
pcCOLspacing = (fh-82.5*2-6*pch)/5;%spacing between top and bottom of any two power cubes in column
corner=36;
%For switches and scales, we assume the center is where we are driving to

n.loc(1,:) = [corner*cos(pi/4),fh-corner*sin(pi/4)];%blue human block source
n.loc(2,:) = [0,fh/2+48/3+12];%red vault sink
n.loc(3,:) = [corner*cos(pi/4),corner*sin(pi/4)];%blue human block source
n.loc(4,:) = [140-42/2,fh/2];%RED pile source
...
n.loc(23,:) = [fw-140+42/2,fh/2];%BLUE pile source
n.loc(24,:) = [fw-corner*cos(pi/4),fh-corner*sin(pi/4)];%RED human block source
n.loc(25,:) = [fw,fh/2+48/3+12];%BLUE vault sink
n.loc(26,:) = [fw-corner*cos(pi/4),corner*sin(pi/4)];%RED human block source
